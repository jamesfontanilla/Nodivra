      ...
      <div className="flex h-full flex-col rounded-[1.625rem] bg-ink-950/88 p-5 shadow-[inset_0_1px_0_rgba(255,255,255,0.08)] sm:p-6">
        ...
        <div className="mt-auto space-y-4 pt-6">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            {service.contactCtaUrl ? <Link href={`/u/${profileHandle}?service=${service.slug}#inbox`} className="inline-flex items-center gap-2 rounded-full bg-white/5 px-4 py-2.5 text-sm text-sand-50 ring-1 ring-white/10 hover:bg-white/10">{service.contactCtaLabel || "Start a conversation"}</Link> : null}
          </div>
        </div>
      </div>
    </article>
...
function AvailabilityBanner({ availability, profileHandle }: { availability: PublicAvailabilitySnapshot; profileHandle: string }) {
  return (
    <div className="rounded-[2rem] bg-sand-100/10 p-1.5 ring-1 ring-sand-100/20">
      <div className="flex flex-col gap-5 rounded-[1.625rem] bg-ink-950/88 p-5 sm:flex-row sm:items-center sm:justify-between sm:p-6">
        ...
        {availability.contactCtaUrl ? <Link href={`/u/${profileHandle}#inbox`} className="inline-flex shrink-0 items-center justify-center gap-2 rounded-full bg-sand-100 px-4 py-2.5 text-sm font-medium text-ink-950 hover:bg-sand-50">{availability.contactCtaLabel || "Start a conversation"}<ArrowUpRightIcon className="h-3.5 w-3.5" /></Link> : null}
      </div>
    </div>
  );
}
...
export function PublicWork({ availability, services, projects, profileHandle }: { availability: PublicAvailabilitySnapshot | null; services: PublicWorkServiceSnapshot[]; projects: PublicProjectSnapshot[]; profileHandle: string }) {
  ...
  return (
    <section className="space-y-5">
      {availability ? <AvailabilityBanner availability={availability} profileHandle={profileHandle} /> : null}
...
export function ServiceArticle({ service, projects, profileHandle, relatedServices = [] }: { service: PublicWorkServiceSnapshot; projects: PublicProjectSnapshot[]; profileHandle: string; relatedServices?: PublicWorkServiceSnapshot[] }) {
  return <article className="space-y-8"><header className="rounded-[2rem] bg-white/5 p-1.5 ring-1 ring-white/10"><div className="rounded-[1.625rem] bg-ink-950/88 p-6 sm:p-10"><div className="flex flex-wrap gap-2"><Badge tone={statusTone(service.availabilityStatus)}>{workAvailabilityLabel(service.availabilityStatus)}</Badge>{service.isFeatured ? <Badge tone="accent">Featured service</Badge> : null}</div><h1 className="mt-5 max-w-4xl font-display text-5xl leading-[0.98] tracking-tight text-sand-50 sm:text-7xl">{service.title}</h1><p className="mt-6 max-w-3xl text-lg leading-8 text-sand-200/85">{service.description}</p><div className="mt-8 flex flex-wrap gap-2">{service.contactCtaUrl ? <Link href={`/u/${profileHandle}?service=${service.slug}#inbox`} className="inline-flex items-center gap-2 rounded-full bg-sand-100 px-4 py-2.5 text-sm font-medium text-ink-950 hover:bg-sand-50">{service.contactCtaLabel || "Start a conversation"}<ArrowUpRightIcon className="h-3.5 w-3.5" /></Link> : null}</div></div></header><div className="grid gap-5 md:grid-cols-2"><div className="rounded-[2rem] bg-white/5 p-1.5 ring-1 ring-white/10"><div className="rounded-[1.625rem] bg-ink-950/88 p-6"><p className="text-xs uppercase tracking-[0.18em] text-sand-300/60">Starting point</p><p className="mt-3 font-display text-3xl text-sand-50">{service.startingPriceText || "Contact for estimate"}</p></div></div><div className="rounded-[2rem] bg-white/5 p-1.5 ring-1 ring-white/10"><div className="rounded-[1.625rem] bg-ink-950/88 p-6"><p className="text-xs uppercase tracking-[0.18em] text-sand-300/60">Typical pace</p><p className="mt-3 font-display text-3xl text-sand-50">{service.deliveryTimeText || "Scoped together"}</p></div></div></div><div className="mx-auto max-w-3xl space-y-5"><div><Badge tone="muted">What this draws on</Badge><div className="mt-4 flex flex-wrap gap-2">{service.skills.map((skill) => <Badge key={skill} tone="muted">{skill}</Badge>)}</div></div><ServiceLinks service={service} projects={projects} profileHandle={profileHandle} /></div>{relatedServices.length > 0 ? <aside className="space-y-5"><div className="flex items-end justify-between gap-4"><div><Badge tone="muted">Keep exploring</Badge><h2 className="mt-3 font-display text-3xl tracking-tight text-sand-50">Other ways to work together</h2></div><Link href={`/u/${profileHandle}/work`} className="text-xs uppercase tracking-[0.18em] text-sand-300/70 underline decoration-white/20 underline-offset-4">All Work</Link></div><div className="grid gap-5 md:grid-cols-2">{relatedServices.slice(0, 2).map((related) => <ServiceCard key={related.id} service={related} projects={projects} profileHandle={profileHandle} />)}</div></aside> : null}</article>;
}
